...
def hist_data_2d(
    ...
    n = np.histogram2d(h, v, histbins, weights=valid)[0]      # Count of the valid data points in each bin
    valid_sum = np.histogram2d(h[~mf], v[~mf], histbins, weights=data)[0]   # Sum of valid data values in each bin
    
    # mask the zero counts
    masked_sum = np.ma.array(valid_sum, mask= n == 0.)
...
def hist_data_3d(
    ...
    # Flatten the arrays
    h = len_h.flatten() - 1.  # minus one to move two points to (-1,0) and (1,0)
    v = len_v.flatten()
    
    if trip:
        signals = np.ma.array([hist_data_2d(mask_data[i, :, :], h, v, hist_bins) for i in range(nf)])
    else:
